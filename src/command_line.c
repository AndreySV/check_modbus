...
int     check_function_num(modbus_params_t* params)
{
	...
	if (rc) printf("Invalid function number: %d\n", params->nf );
    return rc;
}
...
int     check_source( modbus_params_t* params)
{
	...
	if (cnt>1)
	{
		printf("Several modbus input interfaces were declared\n");
		return 1;
	}
	return 0;
}
...
int     check_format_type(modbus_params_t* params)
{
	...
	if (rc)
	{
		...
		if (params->dump)
			printf("-F (--format) parameter can not be used in dump mode \n");
	}
    return rc;
}
...
int      check_command_line(modbus_params_t* params, int argc, char** argv)
{
    ...
    {
        printf("Not provided or unable to parse host address/serial port name/filename: %s\n", argv[0]);
	...
	{
		printf("The maximal number of registers in one dump is 127\n");
		return RESULT_WRONG_ARG;
	}
